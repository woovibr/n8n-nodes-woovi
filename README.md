...
|  `/v1/pix-keys`  | POST | `Woovi` | `pixKey` (string) — required; `pixKeyType` (string) — required | Create a new pix key. Use `resource=pixKey` + `operation=create` and provide `pixKey` and `pixKeyType`. | Pix Key object (key, type, status, ...)
