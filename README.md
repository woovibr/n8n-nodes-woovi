...
| `/v1/account-register` | POST | `Woovi` | `officialName`, `tradeName`, `taxID` (string) — required; `billingAddress` (object) — required; optional: `documents`, `representatives`, `businessDescription`, `businessProduct`, `businessLifetime`, `businessGoal` | Create account register pre-application for a new company. Use `resource=accountRegister` + `operation=create` and provide required fields. | `{ id, status, ... }` |
| `/v1/account-register` | GET | `Woovi` | `taxID` (string) — optional; if provided, returns the account register found by taxID | Get account registers filtered by taxID (e.g., to retrieve a recently created account) — use `resource=accountRegister` + `operation=getByTaxId` | Array of account registers or single object |
| `/v1/account-register/{id}` | PATCH | `Woovi` | `id` (string) — required; `updateFields` (object) — optional with fields to update (`businessDescription`, `businessGoal`, `representatives`, `documents`, etc.) | Update account register details. Use `resource=accountRegister` + `operation=update` and provide `taxID` as identifier and `updateFields` with changes | Updated account register object |
| `/v1/account-register/{id}` | DELETE | `Woovi` | `taxID` (string) — required | Delete an account register in PENDING status. Use `resource=accountRegister` + `operation=delete` and provide the account register `taxID`. | `{ message: "Account register successfully deleted", accountRegisterId: "..." }` |
| `/v1/account` | POST | `Woovi` | — | Create a new account (duplicates account associated with AppID). Use `resource=account` + `operation=create`. Requires bank account feature to be enabled. | Account object with details |
| `/v1/account/{accountId}/withdraw` | POST | `Woovi` | `accountId` (string) — required; `value` (number, cents) — required | Withdraw from an account. Use `resource=account` + `operation=withdraw` and provide `accountId` and `value` (amount in cents). | `{ withdraw: { account: {...}, transaction: {...} } }` |
