...
| `/v1/charge` | GET | `Woovi` | `start` (string, RFC 3339) — optional; `end` (string, RFC 3339) — optional; `status` (string) — optional; `customer` (string) — optional; `subscription` (string) — optional; `limit` (number) — optional; `skip` (number) — optional | List charges with filters. Use `resource=charge` + `operation=list` and optionally provide filter parameters. | Array of charge objects (id, value, status, ...) |
...
| `/v1/charge/{id}` | GET | `Woovi` | `id` (string) — required (charge ID or correlation ID) | Get a single charge by ID or correlationID. Use `resource=charge` + `operation=get` and provide `chargeId`. | Charge object with details (id, value, status, brCode, ...) |
