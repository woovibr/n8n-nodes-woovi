...
| `/v1/application` | DELETE | `Woovi` | (none) — headers `Authorization` required | Deactivate an application (set `isActive=false`). Use `resource=application` + `operation=delete`. | `{ success: true }` |
