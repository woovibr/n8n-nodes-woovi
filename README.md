...
| `/v1/webhook` | GET | `Woovi` | `url` (string) — optional query to filter webhooks | List webhooks registered for your App; set `resource=webhook` + `operation=listWebhooks` and optionally filter by `url`. | `{ pageInfo, webhooks: [...] }` (paginated list) |
| `/v1/webhook/{id}` | DELETE | `Woovi` | `Webhook ID` (string) — required | Delete a webhook by its ID. Set `resource=webhook` + `operation=deleteWebhook` and provide `webhookId`. | `{ status: '...' }` |
